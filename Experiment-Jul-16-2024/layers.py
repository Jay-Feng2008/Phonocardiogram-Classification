...
class MultiHeadSelfAttention(keras.layers.Layer):
    def __init__(self, d_model, num_heads):
        ...
        self.query_projection = tf.keras.layers.Dense(d_model*self.num_heads)
        self.key_projection = tf.keras.layers.Dense(d_model*self.num_heads)
        self.value_projection = tf.keras.layers.Dense(d_model*self.num_heads)
...
class ConvLayer(keras.layers.Layer):
    def __init__(self, c_in):
        ...
        self.downConv3 = tf.keras.layers.Conv1D(
                                  filters=c_in,
                                  kernel_size=3,
                                  padding='causal',
                                  dilation_rate=2,
                                  activation='relu')
        self.downConv5 = tf.keras.layers.Conv1D(
                                  filters=c_in,
                                  kernel_size=5,
                                  padding='causal',
                                  dilation_rate=2,
                                  activation='relu')
        self.downConv8 = tf.keras.layers.Conv1D(
                                  filters=c_in,
                                  kernel_size=8,
                                  padding='causal',
                                  dilation_rate=2,
    ...
    def call(self, x, **kargs):
        ...
        x = self.downConv3(x)
        # x = self.activation(x)
        # x = tf.keras.layers.concatenate([x, self.maxPool(res)], axis=-2)
        x = res + x
        x = self.norm(x)
#         x = self.dropout(x)
        return x
